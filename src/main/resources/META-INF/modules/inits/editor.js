(function(){
	define(["kissy"],function(Kissy){
		var page = {};
		page.init = function(spec){
			window.EDITOR_CFG = {
		        focused: false,
		        fromTextarea:'#'+spec.id
		    };
			var extools = spec.params.exclude,sizeLimit = spec.params.sizeLimit,uploadUrl = spec.url;
			KISSY.use('editor', function (S, Editor) {
		        ...
		        var fullPlugins = [];
		        if(extools){
		        	var es = extools.split(",");
		        	S.each(es,function(item,i){
			        	if(S.inArray(item,plugins)){
			        		S.each(plugins,function(i,j){
			        			if(i == item){
			        				plugins.splice(j,1);
			        			}
			        		});
		        ...
		        var pluginConfig = {
		            ...
		            'image': {
		                ...
		                //remote:false,
		                upload: {
		                    serverUrl: uploadUrl || "upload.java",
		                    serverParams: {
		                        waterMark: function () {
